...
// unsigned int 无符号整型，取值：0 ~ 2^23 - 1
...
console.log('\n\n示例二:')
{
    // 构建成 10 个 A 组成的Buffer，如果 A 不提供，则以 0 填充
		// 默认情况下，Buffer.alloc 会将 A 转换为 ASCII 码（十六进制） 65 A
    const buf = Buffer.alloc(10, 'A')
    const arr = new Uint8Array(10).fill('A'.charCodeAt()) // Returns the Unicode value of the character at the specified location
...
// TODO::示例四: 如何正确地把 TypedArray 转化为 Buffer
console.log('\n\n示例四:')
{
    function f4() {
